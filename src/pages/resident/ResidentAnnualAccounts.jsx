...
export default function ResidentAnnualAccounts() {
  ...
  const fetchData = async () => {
    try {
      const [p, b, e] = await Promise.all([
        paymentsAPI.getAllSociety().catch(() => paymentsAPI.getAll()),
        bookingsAPI.getAllSociety().catch(() => bookingsAPI.getAll()),
        expensesAPI.getAll(),
      ]);
  ...
  /* ── Dynamic calculations exactly matching Admin Annual Accounts ── */
  ...
  if (loading) {
    return (
      <>
        <PageHeader eyebrow="Resident" title="Annual Accounts" subtitle="Society Financial Overview — Civiora" />
  ...
  return (
    <>
      <PageHeader eyebrow="Resident" title="Annual Accounts" subtitle="Dynamic Society Financial Overview — Civiora" />
      ...
      {/* ── Dynamic Expense Breakdown (Read-Only) ── */}
      ...
      <Card className="overflow-hidden mb-8">
        <table className="admin-table">
          ...
          <tbody>
            {expenses.length === 0 && (
              <tr>
                <td colSpan={3} className="text-center text-brown/40 py-6">No expenses recorded yet</td>
              </tr>
            )}
            {expenses.map((exp) => (
              <tr key={exp._id}>
